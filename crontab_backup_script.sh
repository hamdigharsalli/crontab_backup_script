			...
			if grep -q "sent.*bytes.*received.*bytes.*bytes\/sec" <<< "$first_marker" ; then
				if grep -q "total size is.*speedup is" <<< "$second_marker" ; then
					...
					if [ ${#bytes_backed_up} -ne 0 ]; then
                        ...
                        bytes_backed_up_formatted=`echo $bytes_backed_up \
	...
	if [ -e $BACKUP ]; then
		...
		if grep -q "sent.*bytes.*received.*bytes.*bytes\/sec" <<< "$first_marker" ; then
			if grep -q "total size is.*speedup is" <<< "$second_marker" ; then
				...
				bytes_sent=`cut -d ' ' -f 2 <<< "$first_marker"`
				bytes_rcvd=`cut -d ' ' -f 6 <<< "$first_marker"`
				if [ ${#bytes_sent} -ne 0 ]; then
					if [ ${#bytes_rcvd} -ne 0 ]; then
                        ...
                        bandwidth_accumulator_formatted=`echo $bandwidth_accumulator \
                            | perl -pe '1 while s/(.*)(\d)(\d\d\d)/$1$2,$3/'`
...
function dim_display_on_remote_machine
{
    ...
    if [ $? -eq 0 ]; then
        ...
        # Determine if the screen really is off now.
